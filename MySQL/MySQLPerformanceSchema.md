...
* 在mysql的5.7版本中,性能模式是默认开启的,如果想要显式的关闭的话需要修改配置文件,不能直接进行修改,会报错
...
-- 聚合后的摘要表summary,summary表还可以根据帐号(account),主机(host),程序(program),线程(thread),用户(user)和全局(global)再进行细分
...
* 数据库刚刚初始化并启动时,并非所有instruments(事件采集项,在采集项的配置表中每一项都有一个开关字段,或为YES,或为NO)和consumers(事件类型保存表配置项,为YES就表示对应的表保存性能数据,为NO就表示对应的表不保存性能数据)都启用了,所以默认不会收集所有的事件,需要根据实际情况进行设置,可以使用如下两个语句打开对应的instruments和consumers
...
-- 当配置完成之后可以通过查询events_waits_current表得到当前server状态,该表中每个线程只包含一行数据,用于显示每个线程的最新监视事件
...
* 配置表之间存在相互关联关系,按照配置影响的先后顺序,可添加如下:
